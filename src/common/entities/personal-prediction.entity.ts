...
export class PersonalPrediction {
  ...
  @JoinColumn({ name: 'user_id', foreignKeyConstraintName: 'fk_personal_predictions_user' })
  user: User;

  @Column({ type: 'varchar', length: 255, nullable: true })
  title?: string;

  @Column({ name: 'total_stake', type: 'decimal', precision: 10, scale: 2, nullable: false })
  totalStake: number;

  @Column({ name: 'total_odds', type: 'decimal', precision: 8, scale: 2, nullable: true })
  totalOdds?: number;
  ...
  @Column({ name: 'created_at_12hr_before_match', type: 'boolean', nullable: false, default: false })
