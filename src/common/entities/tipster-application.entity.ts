...
export class TipsterApplication {
  ...
  @JoinColumn({ name: 'user_id', foreignKeyConstraintName: 'fk_tipster_applications_user' })
  user: User;
  ...
  @Column({ name: 'identity_document_type', type: 'varchar', length: 50, nullable: true })
  identityDocumentType?: string;

  @Column({ name: 'proof_of_address_url', type: 'text', nullable: true })
  proofOfAddressUrl?: string;

  @Column({ name: 'payout_method', type: 'varchar', length: 50, nullable: false })
  payoutMethod: string;
  ...
  @JoinColumn({ name: 'reviewed_by', foreignKeyConstraintName: 'fk_tipster_applications_reviewed_by' })
  reviewedBy?: User;
